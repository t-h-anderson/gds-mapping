    ...
    methods (Access = private)
        ...
        function buildRulesEditor(obj, parent, row)
            grp = uigridlayout(parent, [2 1], ...
                RowHeight = {32, "1x"}, ColumnWidth = {"1x"});
            ...
            obj.RulesTable = uitable(grp, ...
                ColumnName = {'Kind', 'Priority', 'Pattern / Path', ...
        ...
        function onAddRegex(obj)
            answer = inputdlg( ...
                {'Pattern (regex)', 'Template', 'Priority', 'Notes'}, ...
                'Add Regex Rule', 1, {'^x$', 'y', '10', ''});
            if isempty(answer); return; end
            obj.Session.addRule(eLumina.gds.rules.RegexRule( ...
                Pattern  = string(answer{1}), ...
                Template = string(answer{2}), ...
                Priority = str2double(answer{3}), ...
                Notes    = string(answer{4})));
        end

        function onAddExplicit(obj)
            answer = inputdlg( ...
                {'Simulink path', 'IEC target', 'Priority', 'Notes'}, ...
                'Add Explicit Rule', 1, {'ref1/in1', 'esca_x', '100', ''});
            if isempty(answer); return; end
        ...
        function data = rulesToCell(rules)
            ...
            for k = 1:n
                ...
                if isa(r, "eLumina.gds.rules.RegexRule")
                    data{k, 1} = 'regex';
                    data{k, 3} = char(r.Pattern);
                    data{k, 4} = char(r.Template);
                else
                    data{k, 1} = 'explicit';
                    data{k, 3} = char(r.Path);
                    data{k, 4} = char(r.Target);
                end
                data{k, 2} = r.Priority;
                data{k, 5} = char(r.Notes);
            end
        end
    end
end
